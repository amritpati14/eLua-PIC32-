...
// Remove warning when building with the BUILD_TERM removed.
#ifdef BUILD_TERM
...
{
// PIC32 gcc compiler (XC32) had problems with the original codes initialisation.
#ifdef __PIC32MX
  ARRAYSPEC u32 mstart[1];
  mstart[0] = MEM_START_ADDRESS;
#else
...
// PIC32 gcc compiler (XC32) had problems with the original codes initialisation.
#ifdef __PIC32MX
  ARRAYSPEC u32 mend[1];
  mend[0] = MEM_END_ADDRESS;
#else
