...
#ifdef LUA_META_ROTABLES
#ifdef __PIC32MX
// Adding code for detecting when a pointer on the PIC32 is referencing RAM or Flash.
static char *pstext = NULL;
static char *petext = (char *)START_OF_KSEG1_RAM;
int luaR_isrotable(void *p) {
  return pstext <= ( char* )p && ( char* )p <= petext;
}
#else
extern char stext;
extern char etext;
