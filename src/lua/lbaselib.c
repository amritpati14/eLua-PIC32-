...
// Including the platform configuration header in the PIC32 port even when compiling with the
// LUA_CROSS_COMPILER option enabled.
#if !defined(LUA_CROSS_COMPILER) || defined(__PIC32MX)
...
// Moving variable declaration to start of code block. This is for the Visual Studios C compiler
// which is not C99 complaint, and the original code caused a compilation error. Not wrapping in
// conditional compile as changes are benign and would only have effect on optimiser.
  const char *keyname;
  void *res;
#if LUA_OPTIMIZE_MEMORY == 2
  int fres;
